...
namespace Revit.IFC.Export.Exporter.PropertySet.Calculators
{
   class ConcealedCeilingOffsetCalculator : PropertyCalculator
   {
      ...
      {
         int? concealedCeilingOffset = ParameterUtil.GetIntValueFromElementOrSymbol(element, entryMap.RevitParameterName);
         if (!concealedCeilingOffset.HasValue)
            concealedCeilingOffset = ParameterUtil.GetIntValueFromElementOrSymbol(element, entryMap.CompatibleRevitParameterName);
         if (!concealedCeilingOffset.HasValue)
            return false;
