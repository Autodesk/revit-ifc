...
{
   /// <summary>
   /// A calculation class to calculate the threshold depth of a window or door.
   /// </summary>
   class ThresholdDepthCalculator : PropertyCalculator
   {
      /// <summary>
      /// A double variable to keep the calculated value.
      /// </summary>
      private double m_ThresholdDepth = 0.0;

      /// <summary>
      /// The ThresholdDepthCalculator instance.
      /// </summary>
      public static ThresholdDepthCalculator Instance { get; } = new ThresholdDepthCalculator();

      /// <summary>
      /// Calculates the threshold depth.
      /// </summary>
      ...
      {
         // We only calculate the threshold depth if the threshold thickness is set.
         if (!ThresholdThicknessCalculator.Instance.Calculate(exporterIFC, handle, extrusionCreationData, element, elementType, entryMap))
            return false;

         const string parameterNameV1 = "ThresholdDepth";
         const string parameterNameV2 = "IfcDoorLiningProperties.ThresholdDepth";
         const string parameterNameV3 = "Pset_DoorLiningProperties.ThresholdDepth";
         ...
         if (value.HasValue)
         {
            m_ThresholdDepth = UnitUtil.ScaleLength(value.Value);
            return true;
         }
