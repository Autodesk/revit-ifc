   ...
   public class Exporter : IExporterIFC
   {
      ...
      private void WriteIFCFile(ExporterIFC exporterIFC, Document document)
      {
         ...
         using (IFCTransaction transaction = new IFCTransaction(file))
         {
            ...
            DocumentVersion docVersion = Document.GetDocumentVersion(document);
            if (docVersion != null)
            {
               Guid? versionGUID = docVersion.VersionGUID;
               string versionLine = string.Format("RevitIdentifiers [VersionGUID: {0}, NumberOfSaves: {1}]",
                     versionGUID?.ToString() ?? string.Empty, docVersion.NumberOfSaves);
            ...
            string versionInfos = document.Application.VersionBuild + " - " + 
      ...
      private bool ExportIFCMapConversion(ExporterIFC exporterIFC, Document doc, IFCAnyHandle geomRepContext, IList<double> TNDirRatio)
      {
         ...
         else
         {
            ...
            (double eastings, double northings, double orthogonalHeight, double angleTN, double origAngleTN) geoRefInfo =
                  OptionsUtil.GeoReferenceInformation(doc, wcsBasis, doc.ActiveProjectLocation);
