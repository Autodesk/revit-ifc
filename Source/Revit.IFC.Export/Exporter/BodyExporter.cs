   ...
   class BodyExporter
   {
      ...
      private static bool CategoryHasStructuralMaterialParam(BuiltInCategory categoryId)
      {
         return categoryId == BuiltInCategory.OST_AnalyticalMember ||
            categoryId == BuiltInCategory.OST_AnalyticalPanel ||
      ...
      private static ElementId CalcBestMaterialIdFromParameter(Element element)
      {
         ...
         BuiltInCategory categoryId = element.Category.BuiltInCategory;
      ...
      {
         IDictionary<int, Tuple<List<List<XYZ>>, Color, double>> triangleListDict = 
            new SortedDictionary<int, Tuple<List<List<XYZ>>, Color, double>>();
         ...
         foreach (Face face in faces)
         {
            ...
            int matIdAsInt = matId.IntegerValue;
            if (!triangleListDict.TryGetValue(matIdAsInt, out Tuple<List<List<XYZ>>, Color, double> currList))
            {
               List<List<XYZ>> emptyTriangleList = new List<List<XYZ>>();
               currList = Tuple.Create(emptyTriangleList, color, opacity);
               triangleListDict[matIdAsInt] = currList;
            }
         ...
         foreach (KeyValuePair<int, Tuple<List<List<XYZ>>, Color, double>> data in triangleListDict)
         {
